...
void dir_destroy(Directory *d) {
    ...
    while (d->firstchild_file != NULL) {
        ...
        file_free(f);                               /* AI 增强：连同动态 content 一起释放 */
    }
    ...
    /* 第 3 步：子节点清空后释放自己
     * hash_destroy 只释放桶内链表节点，表本身也是 malloc 的，要补 free */
...
File *file_new(const char *name) {
    ...
    f->content     = calloc(1, f->content_cap);   /* 初始 64 字节，含结尾 '\0' */
    f->content_len = 0;
    return f;
}

/* AI 增强：释放文件动态 content 与节点本体（替代裸 free） */
