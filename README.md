...
### 2026-09-10 — 提交材料：两份项目压缩包 + 报告并列排放

**做了什么：**
1. 打包 `human_version.zip` / `AI_version.zip`，内容只有 `Makefile` + `include/` + `src/`，
   不含任何编译产物（打包前先 `make clean`）。
2. 把实验报告 `202532110113-刘亦轩.docx` 从 `docx/` 移到仓库根目录，与两个压缩包、
   两个源码目录并列；**文件内容未改动**（移动前后 SHA256 一致）。`docx/` 只留老师原始文档与 `.bak` 备份。
3. `.gitignore` 新增 `/*.zip` 与 `/*.docx`：这两类是本地提交材料，防止 `git add -A` 时误入库。

**如何验证：**
- 两个 zip 解压后 `make` 均编译成功；补上 `tests/` 后回放官方样例各输出 5 条 SUCCESS，与仓库内目录行为一致。
- 报告移动前后 SHA256 完全一致（`ef80da0c…`）；`docx/` 里老师原始文档未动。
- 仓库内两版用例仍为 9/9 + 17/17 全绿。

**待确认 / 下一步：**
- 压缩包里只有项目本身，`make run`（回放样例）需要 `tests/` 在同级目录；
  若想让老师「解压即回放样例」，可把 `tests/` 一并打进包里。
